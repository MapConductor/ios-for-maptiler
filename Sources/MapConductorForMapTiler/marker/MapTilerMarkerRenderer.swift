...
@MainActor
final class MapTilerMarkerRenderer: MarkerOverlayRendererProtocol {
    ...
    func onStyleLoaded(_ style: MLNStyle) {
        ...
        MCLog.marker("MapTilerMarkerRenderer.onStyleLoaded")
    ...
    func onAdd(data: [MarkerOverlayAddParams]) async -> [MLNPointFeature?] {
        ...
        ensureDefaultIcon(style: style)
        MCLog.marker("MapTilerMarkerRenderer.onAdd count=\(data.count)")
    ...
    func onChange(data: [MarkerOverlayChangeParams<MLNPointFeature>]) async -> [MLNPointFeature?] {
        guard let style else { return [] }
        ensureDefaultIcon(style: style)
        if !data.isEmpty { MCLog.marker("MapTilerMarkerRenderer.onChange count=\(data.count)") }
    ...
    private func animateMarker(
        ...
        MCLog.marker("MapTilerMarkerRenderer.onAnimate start id=\(entity.state.id) anim=\(animation) bounds=\(String(describing: mapView.bounds))")
        mapView.layoutIfNeeded()
        if mapView.window == nil || mapView.bounds.isEmpty {
            MCLog.marker("MapTilerMarkerRenderer.onAnimate defer id=\(entity.state.id) reason=windowOrBounds")
        ...
        MCLog.marker(
            "MapTilerMarkerRenderer.onAnimate id=\(entity.state.id) targetPoint=\(String(describing: targetPoint)) startPoint=\(String(describing: startPoint)) startBackPoint=\(String(describing: startBackPoint))"
        )

        if !targetPoint.x.isFinite || !targetPoint.y.isFinite || !startBackPoint.x.isFinite || !startBackPoint.y.isFinite {
            MCLog.marker("MapTilerMarkerRenderer.onAnimate defer id=\(entity.state.id) reason=nonFiniteProjection")
            await deferAnimate(entity: entity)
            return
        }
        ...
        if deltaX > 4.0 || deltaY > 4.0 {
            MCLog.marker("MapTilerMarkerRenderer.onAnimate defer id=\(entity.state.id) reason=projectionMismatch dx=\(deltaX) dy=\(deltaY)")
            await deferAnimate(entity: entity)
            return
        }
        ...
        if targetPoint.y > 1,
           ...
           abs(startCoord.longitude - target.longitude) < 1e-10 {
            MCLog.marker("MapTilerMarkerRenderer.onAnimate defer id=\(entity.state.id) reason=projectionNotReady")
            await deferAnimate(entity: entity)
            return
        }
        ...
        let runner = MarkerAnimationRunner(
            ...
            onCompletion: { [weak self] in
                ...
                MCLog.marker("MapTilerMarkerRenderer.onAnimate end id=\(entity.state.id)")
                self?.animateEndListener?(entity.state)
                Task { await self?.onPostProcess() }
            }
        )
    ...
    private func deferAnimate(entity: MarkerEntity<MLNPointFeature>) async {
        ...
        MCLog.marker("MapTilerMarkerRenderer.deferAnimate id=\(id) attempt=\(attempts)")
        guard attempts <= 20 else {
            deferredAnimateAttemptsById.removeValue(forKey: id)
            MCLog.marker("MapTilerMarkerRenderer.deferAnimate id=\(id) givingUp")
    ...
    private func setImage(_ image: UIImage, name: String, style: MLNStyle) {
        // MapTiler treats style images as if they were @1x. To keep icons crisp on Retina
    ...
    private func iconOffset(_ icon: BitmapIcon) -> [NSNumber] {
        // MapTiler style values must be JSON-serializable. `CGVector` / `NSValue` cannot be
